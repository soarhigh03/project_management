...
// 회의록 본문에 삽입하는 이미지 — 스크린샷과 같은 blob 저장소를 공유한다.
// (본문에는 URL만 저장하므로 200KB 본문 한도를 넘지 않는다.)
export async function saveImage(project, dataUrl) {
  return saveScreenshot(project, dataUrl);
}
...
// 태스크 순서 변경: order(taskId 배열) 순으로 재정렬. 누락된 태스크는 기존 순서로 뒤에 붙임.
export async function reorderTasks(project, jobId, order) {
  const job = await loadJob(project, jobId);
  if (!Array.isArray(order)) throw badInput('잘못된 순서입니다.');
  const seen = new Set();
  const next = [];
  ...
  for (const t of job.tasks) if (!seen.has(t.id)) next.push(t);
  job.tasks = next;
  await saveJob(project, job);
  return job;
}
...
export async function deleteMeeting(project, id) {
  // 본문에 삽입된 이미지 blob도 함께 정리 (고아 방지)
  const m = await getMeeting(project, id);
  if (m && m.body) {
    const ids = new Set();
    for (const mm of m.body.matchAll(/[?&]imageId=(s\d+)/g)) ids.add(mm[1]);
    for (const sid of ids) await removeScreenshot(project, sid);
  }
